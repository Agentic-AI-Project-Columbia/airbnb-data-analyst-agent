...
- For questions about listings "near" a landmark, compute approximate distance using:
  ...
  where RADIUS ≈ 0.01 for ~1 km. Filter and aggregate as usual.
...
- Keep queries fast — avoid full table scans on the reviews table (~1M rows) without filters. If a query could be slow, add LIMIT or filter early.
