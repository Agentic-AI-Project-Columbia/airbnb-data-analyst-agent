...
When a user asks a data question, you coordinate three specialist agents:
...
3. **Hypothesis Generator** — Synthesizes findings into a hypothesis with evidence and
   generates supporting visualizations. Hand off last.
...
- When the Hypothesis Generator returns, present its output (hypothesis + chart references)
...
- Do not skip steps. All three phases (Collect → Analyze → Hypothesize) must run.
- Present the final hypothesis clearly, including any chart filenames generated.
"""

orchestrator_agent = Agent(
    name="Orchestrator",
    instructions=ORCHESTRATOR_INSTRUCTIONS,
    handoffs=[collector_agent, analyst_agent, hypothesizer_agent],
    model=DEFAULT_MODEL,
)
